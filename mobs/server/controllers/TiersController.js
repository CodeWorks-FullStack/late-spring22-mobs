...
export class TiersController extends BasController {
  constructor() {
    ...
    this.router
      ...
      .post('', this.create)
      .delete('/:id/close', this.closeTier)
  ...
  async closeTier(req, res, next) {
    try {
      await tiersService.closeTier(req.params.id, req.userInfo.id)
      return res.send("The Tier Is Now Closed")
    } catch (error) {
      next(error)
    }
  }
